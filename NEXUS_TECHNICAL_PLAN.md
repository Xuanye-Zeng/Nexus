...
> **Status:** M1+M2+M3 v1+M5 v1.5+M6 v1 done; Dashboard + /resume + /jobs + /agent live; **Celery 6h auto-ingest live**; **agent_runs observability** live (per-turn latency / status / intent / args, GET /api/agent/runs + stats endpoints); **pytest + GitHub Actions CI live** (101 tests: 72 pure-function pins + 7 PDF parser + 22 invariant-eval gates; ruff lint; frontend tsc + vite build all gating every push/PR); **LLM invariant eval harness live** (`backend/evals/` — cached + live modes, 5 sponsorship invariants × 5 fixtures + 5 bullet_rewriter invariants × 1 fixture, pytest-gated); **PDF export live** (/api/customize-resume/pdf, WeasyPrint pipeline, download button on /resume); **AgentBar rich rendering** (search_jobs / triage_emails / customize_resume intents get structured cards instead of raw JSON). 4 connectors, 8 tools, 4 frontend routes.
