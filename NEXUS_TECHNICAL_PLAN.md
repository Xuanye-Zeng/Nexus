...
|---|---|---|
| 2026-09-02 | 1.8 | **All-cloud LLM routing — Ollama qwen2.5:14b retired from every profile.** M4 Air 24 GB hit amber memory pressure with 10.58 GB swap in active use; `llama-server` alone was resident at 9.85 GB because 4 of 6 `LLM_PROFILES` entries were pinned to qwen2.5:14b (sponsorship / master_intent / master_responder / email). Flipped all six profiles to Groq — the whole point of the factory pattern was making this a one-line swap per task, and it was. Also required: `settings.GROQ_MODEL` default `llama-3.3-70b-versatile` → `openai/gpt-oss-120b` (Groq retired the llama-3.3 SKU late Q3 2026 — surfaced as a 404 on the first agent turn after the switch, updated `.env` + `.env.example`). End-to-end agent turn now runs in **1.94 s** (down from ~30-60 s on-device qwen2.5:14b — Groq streams at 500+ t/s vs ~10 t/s on M4 Air). Batch ingest: 3 listings through the full 5-step pipeline in **3.2 s** total. Fallback for Groq daily-budget squeeze is documented inline: `provider="ollama"` + `model="qwen2.5:3b"` (2-3 GB resident, 4/5 fixture accuracy — known miss on the intern_cpt case) is a per-task drop-in. Verified: 115 pytest still green, live agent turn returns 3 real ranked listings with sponsorship evidence + LCA counts. **Memory freed: 9.29 GB.** |
